...
project_name <- "FATMED" #upper-case
...
cdf_dir <- "i:/SCIENCE-NEXS-NyMetabolomics/Personal folders/Tu/fatmed_cdf/FATMED_urine"
...
spl <- waters_extract_spl(file.path("sample_list",paste0(id_1,".SPL"))) %>%
    select(samplename=FILE_TEXT, filename=FILE_NAME, polarity=MS_FILE) %>% as_tibble()

######tweek urine spl#####
pattern_subject <- "M\\d\\d"
pattern_timepoint <- "W\\d|w\\d|wk\\d"
spl_samplename <- spl$samplename %>% 
    str_replace_all(pattern = " ",replacement = "") %>%
    str_replace_all(pattern = "-",replacement = "") 
subject <- str_extract_all(spl_samplename,pattern_subject,simplify = TRUE) 
timepoint <- str_extract_all(spl_samplename,pattern_timepoint,simplify = TRUE)
spl_1 <- spl %>% mutate(subject,timepoint)
subject_1 <- ifelse(subject =="",yes = spl_samplename,no=subject)
timepoint_1 <- ifelse(timepoint =="",yes = NA,no=timepoint)
...
spl_mode <- spl_1 %>% 
     filter(polarity == mode,
            ...
            subject != "MetStd",
            subject != "MetStd+IntStd") %>% mutate(sample_group = "one")

cdfs <- file.path(cdf_dir,paste0(spl_mode$filename,"01.CDF"))
file.exists(cdfs)
#########define parameters##########
params <- list()
...
data_output_annotation <- annotate_kudb(mz_window,rt_window,polarity = mode,data = data_output) %>% deisotope() %>% select(-isotopes)
