...
function  [data,infosamples,repsamps,missingINsl,missingINd]= Arrange_Data_MZmine2_blank(filename,SampleList,mode,dataform,peakInt,splitter,codenames)
...
mzrt(:,2) = n(:,id2);
t = lower(t);
id = strfind(lower(t(1,:)),lower(peakInt));
id = ~cellfun(@isempty,id); %what does this mean?
data = n(:,id);
file = lower(t(1,id)');
...
%% load sample list and check if something id odd
    ...
    t1n = t1(idinc_notmse,:);    
    % remove blank std
    PSinSampleList = {'metstd','standard'};
